package com.aau.saboteur.ui.theme

import androidx.compose.ui.graphics.Color

val MineCoal = Color(0xFF232526)
val MineSlate = Color(0xFF414345)
val OreGold = Color(0xFFE0B84F)
val OreCopper = Color(0xFFB87333)
val Quartz = Color(0xFFF5F1E8)
val Steel = Color(0xFF8D99AE)
val GlowGold = Color(0xFFFFD166)

// Saboteur Farben
val DarkBrown = Color(0xFF3E2723)
val Parchment = Color(0xFFF4E7C2)
val Gold = Color(0xFFD4AF37)
val MossyGreen = Color(0xFF556B2F)
val BoardGreen = Color(0xFF2D6A2D)
val GlowGreen = Color(0xFF4CAF50)
val FadedRed = Color(0xFFB22C2C)
